...
class Model:
    ...
    def classify_all_symbols(self) -> None:
        ...
        confidence_threshold = 0.7  # Adjust this threshold based on model's performance
...
class View(tk.Frame):
    ...
    """
    # Instance Variables
    select_image_but: tk.Button
    original_image: tk.Label
    detected_image: tk.Label
    convert_but: tk.Button
    code_label: tk.Label
    code_output_label: tk.Label
...
class Controller:
    ...
    def image_select_func(self) -> None:
        """
        Allows the user to set the input image. Assigned to button press.
        """
        filepath = filedialog.askopenfilename()
        if filepath:
            self.model.input_image_path = filepath
            input_image = Image.open(filepath)
            self.model.input_image = input_image

            # Resize image for preview
            resized_image = input_image.resize((300, 300), Image.LANCZOS)
            displayed_image = ImageTk.PhotoImage(resized_image)
            
            # Update view with resized preview image
            self.view.original_image.config(image=displayed_image)
            self.view.original_image.image = displayed_image  # Keep a reference
        
    def find_code_func(self) -> None:
        """
        Utilizes the funcitons in the model to predict symbols and display the neccessary code. Assigned
        to button press.
        """
    ...
    def set_buttons(self) -> None:
        """
        Assigns the functionality of the buttons in the GUI.
        """
        self.view.select_image_but["command"] = self.image_select_func
        self.view.convert_but["command"] = self.find_code_func
